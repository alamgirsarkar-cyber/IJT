...
| OQ-11 | In "pending with", does the employee see a named person or only a role? | **Business** (privacy) | HR Ops + Data Privacy | **Resolved 2026-08-28 (Gate 1 finding G1-F06):** named person only where that person is the employee's own line manager, whom they already know. All other stages show role only. |
| OQ-12 | Who can read the free-text **reason**? It may name a manager or describe a grievance. | **Business** (privacy) | Data Privacy + HR | **Resolved 2026-08-28 (Gate 1 finding G1-F04):** visible to HR Business Partner and the employee only. **Not** visible to either manager. Encrypted at rest, never logged, never in analytics extracts without aggregation. |
...
|---|---|---|---|
| `internal-transfer-request` | Employee-facing request: draft, validate, submit, withdraw, and the employee's view of status and pending actions. Owns the request aggregate and its stage plan. | — | **Approved v1.1** |
