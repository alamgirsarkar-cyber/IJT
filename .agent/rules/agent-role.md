# Agent role — One-Point Employee Portal

Operating procedure for every agent session after the tripwire has sent you here and to
`.agent/rules/governance.md`. Do not copy Gate 1 / Gate 2 sign-off rules into this file.

`.agent/` is the control plane (rules + workflows). `.ai-context/` is the knowledge base.

## Before anything else

1. Read `.agent/rules/governance.md` in full — Gate 1 / Gate 2 roles, DoR/DoD, reviewer
   self-identification, dashboard automation, and write-back live only there.
2. Read `.ai-context/constitution.md` — it overrides every other instruction if they conflict.
3. Read `.ai-context/status.md` — the next piece of work is the first spec that is not Released.
4. Respect `.agent/rules/.agentignore` — do not pull ignored paths into context.

Do not scan the repository. Do not prompt from the spec, the plan, or `prompts.md` as a whole.

## If a reviewer self-identifies

Follow `.agent/rules/governance.md` § Reviewer self-identification. Do not skip the
Artifact / Manual choice. Do not treat chat findings as captured until they are written
back as that file specifies.

## If you are implementing (code)

**Entry is a task ID, not a feature description.** Implementation does not start until
Gate 1 is signed off on the spec per `governance.md`.
...
8. Gate 2: `.agent/workflows/code-review.md`, with sign-off write-back only as
   `governance.md` specifies.
...
| Gate 1 spec review (agent first sweep) | `.agent/workflows/spec-review.md` plus `governance.md` write-back |
| Gate 1/2 reviewer session | `.agent/workflows/gate-review-dashboard.md` |
...
| Gate 2 code review (agent first sweep) | `.agent/workflows/code-review.md` plus `governance.md` write-back |

Discovery still starts in `.ai-context/BRD.md`. A spec is never the first place a
requirement is written down. Shared facts go in `.ai-context/ownership_index.md`.
...
- Duplicate `governance.md` into a tripwire file.
- Fill a Gate 1 outcome as if the named reviewer signed off when they have not.
